...
class ONU(object):
    ...
    def __init__(self, env, M, node_name, verbose):
        ...
        if self.M.G.nodes[self.name][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
            B, t = PPBP.createPPBPTrafficGen(self.M.simTime/0.01, 5, 0.8, 4, self.burstsSourcesCountONU[Globals.TCON1_ID], self.pkt_rate[1])
            self.Bursts[Globals.TCON1_ID].append(B)
            self.Bursts[Globals.TCON1_ID].append(t)
            B, t = PPBP.createPPBPTrafficGen(self.M.simTime/0.01, 5, 0.8, 4, self.burstsSourcesCountONU[Globals.TCON2_ID], self.pkt_rate[1])
            self.Bursts[Globals.TCON2_ID].append(B)
            self.Bursts[Globals.TCON2_ID].append(t)
        else:
            B, t = PPBP.createPPBPTrafficGen(self.M.simTime/0.01, 5, 0.8, 4, self.burstsSourcesCountONU2[Globals.TCON1_ID], self.pkt_rate[1])
            self.Bursts[Globals.TCON1_ID].append(B)
            self.Bursts[Globals.TCON1_ID].append(t)
            B, t = PPBP.createPPBPTrafficGen(self.M.simTime/0.01, 5, 0.8, 4, self.burstsSourcesCountONU2[Globals.TCON2_ID], self.pkt_rate[1])
            self.Bursts[Globals.TCON2_ID].append(B)
            self.Bursts[Globals.TCON2_ID].append(t)
    ...
    def send_report_packet(self):
        ...
        while True:
            ...
            pkt = packet.make_report_packet(
                self.env, dest_node, self.name, neededSizes, self.name, payload='')
