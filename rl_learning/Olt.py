...
import inspect
from cloudpickle.cloudpickle import GLOBAL_OPS
...
class OLT(object):
    ...
    def send_to_odn(self, pkt, dest_node):
        ...
        if  self.M.G.nodes[pkt.get(Globals.SOURCE)][Globals.NODE_TYPE_KWD] == Globals.OLT_TYPE:
            ...
            # self.Grant_message_OLT.append([self.myClock.now, pkt])
            Globals.appendDequeue(self.Grant_message_OLT, [self.myClock.now, pkt], Globals.QUEUE_SIZE_LIMIT, True, False)
    ...
    def initAllocationVars(self):
        self.FB_remaining = Globals.FB
        # print(int(self.myClock.now/0.000125), (self.myClock.now))
        for x in self.M.G.nodes():
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
                if int(self.myClock.now/0.000125) % Globals.URLLC_SI_MIN == 0: # recharge URLLC AB min 
                    self.vb_min_matrix[x][Globals.TCON1_ID] = max(0, Globals.URLLC_AB_MIN)
                    self.vb_max_matrix[x] [Globals.TCON1_ID] = Globals.URLLC_RM

                if int(self.myClock.now/0.000125) % Globals.EMBB_SI_MIN == 0:  # recharge EMBB ABmin
                    self.vb_min_matrix[x][Globals.TCON2_ID] = max(0, Globals.EMBB_AB_MIN)
                    self.vb_max_matrix[x] [Globals.TCON2_ID] = Globals.EMBB_RM

                if int(self.myClock.now/0.000125) % Globals.URLLC_SI_MAX == 0:  # recharge EMBB ABmin
                    self.vb_sur_matrix[x][Globals.TCON1_ID] = max(0, Globals.URLLC_AB_SUR)
                if int(self.myClock.now/0.000125) % Globals.EMBB_SI_MAX == 0:  # recharge EMBB ABmin
    ...
    def generate_grant_msg(self, rlAction, maxAllowedAllocation = -1):
        """Process that generates networks packets"""
        self.initAllocationVars()
        # choose the destination node
        dest_node = Globals.BROADCAST_GRANT_DEST_ID
        # create the packet
        # print ("requested: " + str(self.onu_queue_status))
        # bwm = self.generateGrantMsg(rlAction, maxAllowedAllocation)
        bwm = self.generateGrantMsgSingleService(rlAction, maxAllowedAllocation)
        # print ("allocated: " + str(bwm))
        self.counter += 1
        # print (self.counter)
        # if (bwm['1'][Globagrant message:ls.TCON1_ID] > 0 or bwm['1'][Globals.TCON2_ID]) > 0:
            # print ("bwm: " + str(bwm)) 
        pkt = packet.make_grant_packet(self.myClock.now, self.countergrant, dest_node, self.name, bwm, payload='')
        ...
        self.countergrant = self.countergrant + 1
        return pkt
    ...
    def shouldAllocateOnuContinous (self, rlAction):
        for x in self.M.G.nodes():
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
                if rlAction[0] == 0:
    ...
    def shouldAllocateOnuContinousPerOnuSAC (self, rlAction):
    ...
    def shouldAllocateOnuMultiDiscreteA2C3ACtion (self, rlAction):
        for x in self.M.G.nodes():
            Tcon1 = 2*int(x) - 2
            Tcon2 = 2*int(x) - 1
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
    ...
    def shouldAllocateOnuMultiDiscreteA2CService8Actions (self, rlAction):
        for x in self.M.G.nodes():
            Tcon1 = 2*int(x) - 2
            Tcon2 = 2*int(x) - 1
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
    ...
    def actionToAllocation (self, rlAction):
        for x in self.M.G.nodes():
            Tcon1 = int(x) - 1
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
                if rlAction[Tcon1] == 1:
    ...
    def actionToAllocationCont(self, rlAction):
        for x in self.M.G.nodes():
            Tcon1 = int(x) - 1
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
                action = round(rlAction[Tcon1], 2)
                self.vb_min_matrix[x][Globals.TCON1_ID] =  int(action * 2 * Globals.URLLC_AB_MIN)
                

    def shouldAllocateOnuMultiDiscretePPO2Onu8Actions (self, rlAction):
        for x in self.M.G.nodes():
            Tcon1 = 2*int(x) - 2
            Tcon2 = 2*int(x) - 1
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
    ...
    def shouldAllocateOnuMultiDiscreteDQNOnu8Action (self, rlAction):
        for x in self.M.G.nodes():
            Tcon1 = 2*int(x) - 2
            Tcon2 = 2*int(x) - 1
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
    ...
    def getRlState(self):
        ...
        for x in self.M.G.nodes():
            ...
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
                ...
                state[3][id] = 0
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE2:
                state[2][id] = self.onu_queue_status[x][Globals.TCON1_ID]
                state[3][id] = self.onu_queue_status[x][Globals.TCON2_ID]
                state[0][id] = 0
                state[1][id] = 0
        return state
    ...
    def generateGrantMsgSingleService(self, rlAction, maxAllowedAllocation = -1):
        # print(self.myClock.now)
        Globals.appendDequeue( self.action_all, rlAction, Globals.QUEUE_SIZE_LIMIT, True, False)
        
        # self.actionToAllocation(rlAction)
        self.actionToAllocationCont(rlAction)

        # if (maxAllowedAllocation != -1):
        #     self.FB_remaining = maxAllowedAllocation
        ...
        if (total <= self.FB_remaining):
            ...
            temp = copy.deepcopy(self.vb_min_matrix)
            while(self.FB_remaining > 0):
                count = 0
                for x in self.M.G.nodes():
                    if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE: 
                        if (temp[x][Globals.TCON1_ID] > 0):
                            count = count + 1
                if (count == 0):
                    break
                minURLLC = int(self.FB_remaining / count)
                if (minURLLC < count):
                    break
                for x in self.M.G.nodes():
                    if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
                        val = min(temp[x][Globals.TCON1_ID], minURLLC) 
                        allocationResult[x][Globals.TCON1_ID] = allocationResult[x][Globals.TCON1_ID] + val
                        temp[x][Globals.TCON1_ID] = temp[x][Globals.TCON1_ID] - val
                        self.FB_remaining = self.FB_remaining - val
            
        for x in self.M.G.nodes():
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
                self.onu_queue_status[x][Globals.TCON1_ID] = max(self.onu_queue_status[x][Globals.TCON1_ID] - allocationResult[x][Globals.TCON1_ID],0)
    ...
    def generateGrantMsg(self, rlAction, maxAllowedAllocation = -1):
        # print(self.myClock.now)
        Globals.appendDequeue( self.action_all, rlAction, Globals.QUEUE_SIZE_LIMIT, True, False)
        
        # self.action_all.append(rlAction)
        # # self.shouldAllocateOnuContinous(rlAction)
        # self.shouldAllocateOnuContinousPerOnuSAC(rlAction)
        # self.shouldAllocateOnuMultiDiscreteA2C(rlAction)
        # self.shouldAllocateOnuMultiDiscreteA2CService3Acrions(rlAction)
        # self.shouldAllocateOnuMultiDiscreteA2C6ACtions(rlAction)
        # self.shouldAllocateOnuMultiDiscreteA2CService8Actions(rlAction)
        # self.shouldAllocateOnuMultiDiscretePPO2Onu8Actions(rlAction)
        # self.actionToAllocation(rlAction)
        self.actionToAllocationCont(rlAction)
        # self.shouldAllocateOnuMultiDiscreteDQNOnu8Action(rlAction)
        ...
        for x in self.M.G.nodes():
            if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE:
                self.onu_queue_status[x][Globals.TCON1_ID] = max(self.onu_queue_status[x][Globals.TCON1_ID] - allocationResult[x][Globals.TCON1_ID],0)
                self.onu_queue_status[x][Globals.TCON2_ID] = max(self.onu_queue_status[x][Globals.TCON2_ID] - allocationResult[x][Globals.TCON2_ID],0)
    ...
    def allocateRemaining(self, rlAction):
        ...
        if self.FB_remaining == 0:
            ...
            while (self.FB_remaining > 0 and (int(self.myClock.now/0.000125) % Globals.IP_SI_MIN == 0 or int(self.myClock.now/0.000125) % Globals.VIDEO_SI_MIN == 0 or int(self.myClock.now/0.000125) % Globals.URLLC_SI_MIN == 0 or int(self.myClock.now/0.000125) % Globals.EMBB_SI_MIN==0 )):
                urllc_onu_count = 0
                embb_onu_count = 0
                video_onu_count = 0
                ip_onu_count = 0
                ...
                for x in self.M.G.nodes():
                    if (not self.shouldAllocateOnu(rlAction, x, Globals.TCON1_ID) and not self.shouldAllocateOnu(rlAction, x, Globals.TCON2_ID)):
                            continue
                    if self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE2:
                ...
                while self.FB_remaining - 4*y < 0:
                    y = int(y - 1)
                # print(y, onu_type1_count, self.myClock.now)

                for x in self.M.G.nodes():
                    if  (self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE and self.onu_queue_status[x][Globals.TCON1_ID] > 0 and self.FB_remaining > 0 and int(self.myClock.now/0.000125) % Globals.URLLC_SI_MIN == 0):
                    ...
                    if (self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE and self.onu_queue_status[x][Globals.TCON1_ID] == 0 and self.FB_remaining > 0 and int(self.myClock.now/0.000125) % Globals.URLLC_SI_MIN == 0):    
                        newValue = int(y / onu_type1_count) if onu_type1_count > 0 else 0
                        while self.FB_remaining - newValue < 0:
                            newValue = newValue -1
                    ...
                    if  (self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE and self.onu_queue_status[x][Globals.TCON2_ID] > 0 and self.FB_remaining > 0 and  int(self.myClock.now/0.000125) % Globals.EMBB_SI_MIN == 0):
                    ...
                    if  (self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE2 and self.onu_queue_status[x][Globals.TCON1_ID] > 0 and self.FB_remaining > 0 and int(self.myClock.now/0.000125) % Globals.VIDEO_SI_MIN == 0):
                    ...
                    if  (self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE2 and self.onu_queue_status[x][Globals.TCON2_ID] > 0 and self.FB_remaining > 0 and int(self.myClock.now/0.000125) % Globals.IP_SI_MIN == 0):
                    ...
                    if (self.M.G.nodes[x][Globals.NODE_TYPE_KWD] == Globals.ONU_TYPE2 and self.onu_queue_status[x][Globals.TCON2_ID] == 0 and self.FB_remaining > 0 and int(self.myClock.now/0.000125) % Globals.IP_SI_MIN == 0):
                        ...
                        # self.vb_max_matrix[x][Globals.TCON2_ID] = self.vb_max_matrix[x][Globals.TCON2_ID] - newValue
                        self.onu_queue_status[x][Globals.TCON2_ID] = self.onu_queue_status[x][Globals.TCON2_ID] - newValue if self.onu_queue_status[x][Globals.TCON2_ID] > 0 else 0
                while (self.FB_remaining > 0 and int(self.myClock.now/0.000125) % Globals.IP_SI_MIN == 0):
                    for x in self.M.G.nodes():
                        if self.FB_remaining < 0:
    ...
    def allocateAssuredUrllcRL(self, rlAction):
            allocationResult = self.initAllocationDict()
            if (self.FB_remaining <= 0):
                return allocationResult
            # FB > 0
    ...
    def queue_monitor(self):
        ...
        while True:
            ...
            # self.queue_mon.append([self.myClock.now, len(self.proc_queue)])
            Globals.appendDequeue(  self.queue_mon, [self.myClock.now, len(self.proc_queue)], Globals.QUEUE_SIZE_LIMIT, True, False)
    ...
    #     allocationResult = self.initAllocationDict()
    #     allocationResult = self.allocateAssuredTcon2()
    #     tmp = self.allocateAssuredTcon3()
    #     allocationResult = self.aggregateAllocation(allocationResult, tmp)
    #     tmp = self.allocateTcon4()
    #     allocationResult = self.aggregateAllocation(allocationResult, tmp)
    #     tmp = self.allocateSurplusTcon3()
    #     allocationResult = self.aggregateAllocation(allocationResult, tmp)
    #     return allocationResult
    ...
    #                 self.onu_queue_status[x][Globals.TCON1_ID] = self.onu_queue_status[x][Globals.TCON1_ID] - newValue 
    #     else:
    #         while (self.FB_remaining > 0 and tcon1_2_requested_sum > 0):
    ...
    #                 self.onu_queue_status[x][Globals.TCON1_ID] = self.onu_queue_status[x][Globals.TCON1_ID] - newValue
    #     else:
    #         while (self.FB_remaining > 0 and (tcon1_3_requested_sum + tcon2_3_requested_sum) > 0):
    ...
    #                 self.onu_queue_status[x][Globals.TCON2_ID] = self.onu_queue_status[x][Globals.TCON2_ID] - newValue
    #     else:
    #         while (self.FB_remaining > 0 and tcon2_4_requested_sum > 0):
