...
class env_dba(gym.Env):
    ...
    def __init__(self, network, M):
        ...
        n_actions = ()
        for x in range(self.onus_count * 2):
            n_actions = n_actions + (8,)
        self.action_space = spaces.Discrete(8**(self.onus_count*2))
        self.discreteToMultiDiscMap = tuple(numpy.ndindex(n_actions))
        self.observation_space = spaces.Box(low=Globals.OBSERVATION_MIN, high=Globals.OBSERVATION_MAX, shape=(self.serviceCount, self.onus_count), dtype=numpy.float32)
        ...
        self.state = numpy.array([numpy.repeat(0, self.onus_count),numpy.repeat(0, self.onus_count)], numpy.float32)
    ...
    def normalizeState(self,state):
        ...
        for onu in self.onu1_array:
            id = int(onu.name)
            if (tmpState[0][id-1] < Globals.OBSERVATION_MIN):
                tmpState[0][id-1] = Globals.OBSERVATION_MIN
            if (tmpState[0][id-1] > Globals.OBSERVATION_MAX):
                tmpState[0][id-1] = Globals.OBSERVATION_MAX
            if (tmpState[1][id-1] < Globals.OBSERVATION_MIN):
                tmpState[1][id-1] = Globals.OBSERVATION_MIN
            if (tmpState[1][id-1] > Globals.OBSERVATION_MAX):
                tmpState[1][id-1] = Globals.OBSERVATION_MAX
            
            tmpState[0][id-1] = tmpState[0][id-1] / Globals.OBSERVATION_MAX
            tmpState[1][id-1] = tmpState[1][id-1] / Globals.OBSERVATION_MAX
        ...
        for onu in self.onu2_array:
            ...
            if (tmpState[1][id-1] < Globals.OBSERVATION_MIN):
                tmpState[1][id-1] = Globals.OBSERVATION_MIN
            if (tmpState[1][id-1] > Globals.OBSERVATION_MAX):
                tmpState[2][id-1] = Globals.OBSERVATION_MAX
            tmpState[0][id-1] = tmpState[0][id-1] / Globals.OBSERVATION_MAX
            tmpState[1][id-1] = tmpState[1][id-1] / Globals.OBSERVATION_MAX
    ...
    def updateStateFromGrant(self, grantPkt):
        ...
        for onu in self.onu1_array:
            ...
            tmpState[1][id-1] = self.state[1][id-1] - grantPkt[onu.name][Globals.TCON2_ID]
    ...
    def updateStateRandom(self):
        ...
        for onu in self.onu1_array:
            id = int(onu.name)
            self.state[0][id-1] = random.randint(min, max)
            self.state[1][id-1] = random.randint(min, max)
