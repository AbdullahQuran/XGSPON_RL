...
class env_dba(gym.Env):
    ...
    def __init__(self, network, M):
        ...
        self.action_space = spaces.Box(low=0.0, high=1.0, shape=(self.onu_type1_count, ),  dtype=numpy.float32)

        # observation_space = [URRLC_SUM, EMBB_SUM, VIDEO_SUM, IP_SUM]
        self.observation_space = spaces.Box(low=self.minObservation, high=self.maxObservation, shape=(1, self.onu_type1_count), dtype=numpy.float32)
        ...
        self.allocationArray = numpy.empty(self.onu_type1_count, dtype=numpy.float32)
    ...
    def step(self, action):
        ...
        reward, done1 = self.computeReward2()
    ...
    def multiDiscreteToAllocation (self, rlAction):
        for i in range(self.onu_type1_count):
            self.allocationArray[i] = self.actionToVal(rlAction[i], Globals.URLLC_AB_MIN)
        self.allocationArray = numpy.array(self.allocationArray, dtype=numpy.float32)


    def deNormalizeState(self,state):
        for i in range(self.onu_type1_count):
            state[i] = state[i] * self.maxObservationURLLC
        return state
    ...
    def normalizeState(self, state, maintainBounds = True):
        for i in range(self.onu_type1_count):
            if (maintainBounds):
                ...
                if (state[i] > self.maxObservationURLLC):
                    state[i] = self.maxObservationURLLC
            state[i] = state[i]/self.maxObservationURLLC
